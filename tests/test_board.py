...
def test_render_parse_roundtrip():
    cols = {
        "Today": [B.Card(title="Two Sum", line="solve it + jot the hash-map trick",
                         outcome="ah.apply", course="GEN101", tier="easy")],
        "Done": [B.Card(title="Big-O", line="derive the bound", outcome="bo.apply",
                        course="GEN101", checked=True)],
    }
    md = B.render_board(cols)
    assert "kanban-plugin: board" in md and "## Today" in md and "## Proof Pending" in md
    parsed = B.parse_board(md)
    t = parsed["Today"][0]
    assert t.title == "Two Sum" and t.outcome == "ah.apply" and t.course == "GEN101" and t.tier == "easy"
...
def test_hidden_metadata_not_shown_as_title():
    md = B.render_board({"Today": [B.Card(title="Ship an agent", outcome="s1.finals", course="AG201")]})
