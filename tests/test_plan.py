...
def test_difficulty_is_keyed_by_topic_not_unit_id():
    ...
    m = recompute(LearnerModel(), [rec("f1.apply", 0.78, course="GEN101", tier="hard")],
                  tz="UTC", now=dt("2026-07-06T21:00:00"))
    assert difficulty_for(m, "basics") == "easy"                    # unit id misses → baseline (the bug)
    assert difficulty_for(m, topic_of_outcome("f1.apply")) == "med"  # topic key → elevated off baseline
...
def test_plan_cli_reports_elevated_difficulty(tmp_path, capsys):
    ...
    append_record(tmp_path / "records" / "grades.jsonl",
